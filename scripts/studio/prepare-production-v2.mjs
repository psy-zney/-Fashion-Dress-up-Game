...
// The modal top is worn outside the jeans. Narrow only the covered waistband
// rows to 92%; restore the original trousers immediately below the shirt edge
// so the hips and legs still cover the model exactly.
const jeansUnderYellow = Buffer.alloc(jeans.length);
const waistCenter = 523, coveredWaistBottom = 646;
for (let y = 0; y < H; y++) {
  const scaleX = y <= coveredWaistBottom ? 0.92 : 1;
  for (let x = 0; x < W; x++) {
    const sourceX = waistCenter + (x - waistCenter) / scaleX;
    if (sourceX < 0 || sourceX >= W - 1) continue;
    const left = Math.floor(sourceX), right = left + 1, mix = sourceX - left;
    const outputPixel = (y * W + x) * 4;
    for (let channel = 0; channel < 4; channel++) {
      const leftValue = jeans[(y * W + left) * 4 + channel];
      const rightValue = jeans[(y * W + right) * 4 + channel];
      jeansUnderYellow[outputPixel + channel] = Math.round(leftValue * (1 - mix) + rightValue * mix);
    }
  }
}
...
await fs.writeFile(`${out}/ready.json`, JSON.stringify({ version: 'production-v2-20260909-v13-narrow-waistband', catalogGarments: items.length, items, poses: ['top-fitted-denim-pose', 'top-modal-grommet-pose'], variants: ['bottom-sculpted-jeans-under-yellow'], updatedAt: new Date().toISOString() }, null, 2) + '\n');
const report = [];
